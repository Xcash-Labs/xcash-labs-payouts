...
static response_t* send_to_one_host(const char* host, int port,
  ...
  DEBUG_PRINT("Host:%s | %s | %lds",
              host,
              r->status == STATUS_OK ? "OK" : r->status == STATUS_TIMEOUT ? "TIMEOUT"
                                                                          : "ERROR",
...
void cleanup_responses(response_t** responses) {  // definition (non-static)
  if (!responses) return;
  for (size_t i = 0; responses[i] != NULL; ++i) {
    free(responses[i]->host);
    free(responses[i]->data);  // OK if NULL
    free(responses[i]);
  }
  free(responses);
}