...
bool is_blockchain_synced(char *target_height, char *height)
{
      ...
      parse_json_data(response, "result.offline",       offline_flag, sizeof(offline_flag)) != 0)
  {
    ...
    DEBUG_PRINT("Daemon not yet synced or status not OK: synchronized=%s, status=%s", synced_flag, status_flag);
    DEBUG_PRINT("Daemon not yet synced or status not OK: offline=%s, busy_syncing=%s", offline_flag, bs_flag);
    return false;
  }
