...
int sign_data(char *message)
{
  ...
  // Build the core message without signature
  ...
  // Escape result string for HTTP-based signing (if needed)
  string_replace(result, MAXIMUM_AMOUNT, "\"", "\\\"");

  bool use_vrf_signing = 
  ...
  if (use_vrf_signing) {
    ...
    // Build JSON-RPC request to wallet RPC
    ...
    if (send_http_request(data, XCASH_WALLET_IP, "/json_rpc", XCASH_WALLET_PORT, "POST",
                          HTTP_HEADERS, HTTP_HEADERS_LENGTH, string,
