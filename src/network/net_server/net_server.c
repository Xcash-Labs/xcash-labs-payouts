...
void* handle_client(void* arg) {
  ...
  unsigned char stack_buf[BUFFER_SIZE];
  unsigned char* agg = NULL;
  size_t agg_len = 0, agg_cap = 0;
  int saw_first_chunk = 0;

  for (;;) {
    ssize_t n = recv(client->socket_fd, stack_buf, sizeof(stack_buf), 0);

    if (n < 0) {
      if (errno == EINTR) continue;
      if (errno == EAGAIN || errno == EWOULDBLOCK) break; // timeout -> process whatever we have
      // hard error
      WARNING_PRINT("recv error from %s (errno=%d)", client->client_ip, errno);
      goto cleanup;
    }

    if (n == 0) {
      // EOF: sender closed — we're done reading
      break;
    }

    if (!saw_first_chunk) {
      saw_first_chunk = 1;
      if ((n >= 4 && (!memcmp(stack_buf, "GET ", 4) || !memcmp(stack_buf, "POST", 4) || !memcmp(stack_buf, "HEAD", 4)))) {
        WARNING_PRINT("Rejected HTTP request from %s", client->client_ip);
        goto cleanup;
      }
    }

    // grow agg buffer if needed
    if (agg_len + (size_t)n > agg_cap) {
      size_t ncap = agg_cap ? agg_cap * 2 : 8192;
      while (ncap < agg_len + (size_t)n) ncap <<= 1;
      unsigned char* tmp = (unsigned char*)realloc(agg, ncap);
      if (!tmp) {
    ...
    memcpy(agg + agg_len, stack_buf, (size_t)n);
    agg_len += (size_t)n;
  }
  ...
  if (agg_len > 0) {
    ...
    if (!decompress_gzip_with_prefix((const unsigned char*)agg, agg_len, &decompressed, &decompressed_len)) {
      WARNING_PRINT("Failed to decompress message from %s", client->client_ip);
      goto cleanup;
    }
