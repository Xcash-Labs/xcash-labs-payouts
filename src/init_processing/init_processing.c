...
// --- replace your current loop with this minimal, compiling instrumentation ---

updpops_entry_t allowed[16];   // bump cap a bit for safety
size_t allowed_n = 0;
...
for (i = 0; endpoints[i]; ++i) {
  ...
  // Minimal instrumentation using only existing functions/types:
  INFO_PRINT("[DNSSEC] endpoint=%s accepted_entries=%zu", endpoints[i], m);
  for (size_t j = 0; j < m; ++j) {
    INFO_PRINT("[DNSSEC] %s[%zu]: name=%s channel=%s version=%s digest=%s",
               endpoints[i], j,
               tmp[j].name, tmp[j].channel, tmp[j].version, tmp[j].digest);
  }

  if (m == 0) {
    WARNING_PRINT("[DNSSEC] No entries from %s; will skip in merge (check hostname and zone)", endpoints[i]);
    continue; // don't fail; we’ll merge what we do have
  }
  ...
  if (!have_base) {
    ...
    if (!same_set_by_digest(base, base_n, tmp, m)) {
      WARNING_PRINT("Mirror mismatch: %s (baseline) != %s; continuing with merge", base_src, endpoints[i]);
    }
  }

  // Merge into global allowed[] with dedupe by digest
...
// proceed: get_self_sha256(), digest_allowed(), newer-version warning...
