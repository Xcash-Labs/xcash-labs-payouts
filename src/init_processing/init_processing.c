...
static size_t dnssec_get_all_updpops_debug(dnssec_ctx_t* ctx,
                                           const char* host,
                                           updpops_entry_t* out,
                                           size_t cap)
{
  // 1) fetch raw TXT strings with DNSSEC status
  dnssec_txt_result_t r = {0};
  dnssec_status_t st = dnssec_fetch_txts(ctx, host, &r); // implement/unwrap from your ctx
  INFO_PRINT("[DNSSEC] host=%s status=%d txt_count=%zu", host, (int)st, r.count);

  if (st != DNSSEC_SECURE) {
    WARNING_PRINT("[DNSSEC] Non-SECURE status for %s (status=%d). Continuing for debug.", host, (int)st);
    // NOTE: If your production path requires SECURE, keep that check there. Here we just log.
  }

  for (size_t i = 0; i < r.count; ++i) {
    // If your library returns multi-chunk TXT, concatenate before printing.
    const char* s = r.txt[i];
    INFO_PRINT("[DNSSEC][%s] TXT[%zu]=%s", host, i, s ? s : "(null)");
  }

  // 2) parse into updpops entries with explicit reasons for any drop
  size_t n = 0;
  for (size_t i = 0; i < r.count; ++i) {
    updpops_entry_t e = {0};
    if (!r.txt[i]) { WARNING_PRINT("[DNSSEC][%s] TXT[%zu] is null", host, i); continue; }
    if (!parse_updpops_token(r.txt[i], &e)) {
      WARNING_PRINT("[DNSSEC][%s] TXT[%zu] rejected by parser", host, i);
      continue;
    }
    if (n < cap) {
      out[n++] = e;
    } else {
      WARNING_PRINT("[DNSSEC][%s] capacity %zu reached; extra entries ignored", host, cap);
      break;
    }
  }

  // 3) free any r.* resources if your fetch allocates (not shown)
  dnssec_free_txts(&r);

  INFO_PRINT("[DNSSEC] host=%s accepted=%zu", host, n);
  return n;
}
