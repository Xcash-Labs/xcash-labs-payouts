...
static int total_delegates = 0;          if (update_stats) {
            ...
            // ** update the consensus_rounds collection **
            {
              ...
              if (!hex_to_byte_array(previous_block_hash, prev_hash_bin, sizeof prev_hash_bin) ||
                  ...
                  !hex_to_byte_array(final_vote_hash_hex, vote_hash_bin, sizeof vote_hash_bin)) {
              ...
              uint32_t out = 0;
              for (uint32_t i = 0; i < BLOCK_VERIFIERS_AMOUNT; ++i) {
                ...
                if (!hex_to_byte_array(current_block_verifiers_list.block_verifiers_public_key[i], pk_bin, 32) ||
                    !hex_to_byte_array(current_block_verifiers_list.block_verifiers_vrf_proof_hex[i], proof_bin, 80) ||
                    !hex_to_byte_array(current_block_verifiers_list.block_verifiers_vrf_beta_hex[i], beta_bin, 64)) {
                  WARNING_PRINT("[round write] verifier hex→bin decode failed (i=%u) height=%llu",
                                i, (unsigned long long)cbheight);
                  continue;  // skip this entry if it’s malformed
                }

                bson_t item;
                char k[16];
                bson_uint32_to_string(out, NULL, k, sizeof k);
                bson_append_document_begin(&arr, k, -1, &item);
                BSON_APPEND_UTF8(&item, "public_address", addr);  // keep as string
...
// Only update statics and round on seed nodes
#ifdef SEED_NODE_ON
          ...
          if (update_stats) {
            ...
            unsigned long long cbheight = strtoull(current_block_height, NULL, 10);
            mongoc_client_t* c = mongoc_client_pool_pop(database_client_thread_pool);
            if (!c) { ERROR_PRINT("mongo client pop failed"); return; }
