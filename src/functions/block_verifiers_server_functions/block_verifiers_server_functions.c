...
void server_receive_data_socket_block_verifiers_to_block_verifiers_vrf_data(const char* MESSAGE)
{
  ...
  if (hex_to_byte_array(vrf_secret_key_data, vrf_secret_key, crypto_vrf_SECRETKEYBYTES) != XCASH_OK) {
    ERROR_PRINT("Failed to parse vrf_secret_key_data into bytes.");
    return;
  }
  
  if (hex_to_byte_array(vrf_public_key_data, vrf_public_key, crypto_vrf_PUBLICKEYBYTES) != XCASH_OK) {
  ...
  for (count = 0; count < BLOCK_VERIFIERS_AMOUNT; count++) {
    if (strncmp(current_block_verifiers_list.block_verifiers_public_address[count], public_address, XCASH_WALLET_LENGTH) == 0 &&
      ...
      memcpy(VRF_data.block_verifiers_vrf_proof_data[count], vrf_proof, VRF_PROOF_LENGTH);
      memcpy(VRF_data.block_verifiers_vrf_beta_data[count], vrf_beta, VRF_BETA_LENGTH);
    }
  }
