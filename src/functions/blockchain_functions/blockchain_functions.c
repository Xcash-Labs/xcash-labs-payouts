...
int network_block_string_to_blockchain_data(const char* DATA, const char* BLOCK_HEIGHT, const int BLOCK_VERIFIERS_TOTAL)
{
  ...
  memset(block_height,0,sizeof(block_height));
  memset(data2,0,sizeof(data2));
  memset(&blockchain_data, 0, sizeof(blockchain_data));
  ...
  count+= blockchain_data.timestamp_data_length;
  memcpy(blockchain_data.timestamp_data,&DATA[count-blockchain_data.timestamp_data_length],blockchain_data.timestamp_data_length);
  tmp_value = strtol(blockchain_data.timestamp_data, NULL, 16);
  if (tmp_value < 0) return XCASH_ERROR;
  blockchain_data.timestamp = varint_decode((size_t)tmp_value);
  ...
  count+= blockchain_data.unlock_block_data_length;
  memcpy(blockchain_data.unlock_block_data,&DATA[count-blockchain_data.unlock_block_data_length],blockchain_data.unlock_block_data_length);
  tmp_value = strtol(blockchain_data.unlock_block_data, NULL, 16);
  if (tmp_value < 0) return XCASH_ERROR;
  blockchain_data.unlock_block = varint_decode((size_t)tmp_value);
  ...
  count+= blockchain_data.block_height_data_length;
  memcpy(blockchain_data.block_height_data,&DATA[count-blockchain_data.block_height_data_length],blockchain_data.block_height_data_length);
  tmp_value = strtol(blockchain_data.block_height_data, NULL, 16);
  if (tmp_value < 0) return XCASH_ERROR;
  blockchain_data.block_height = varint_decode((size_t)tmp_value);
  ...
  count+= blockchain_data.block_reward_data_length;
  memcpy(blockchain_data.block_reward_data,&DATA[count-blockchain_data.block_reward_data_length],blockchain_data.block_reward_data_length);
  tmp_value = strtol(blockchain_data.block_reward_data, NULL, 16);
  if (tmp_value < 0) return XCASH_ERROR;
  blockchain_data.block_reward = varint_decode((size_t)tmp_value);
  ...
  count+= blockchain_data.extra_bytes_size_data_length;
  memcpy(blockchain_data.extra_bytes_size_data,&DATA[count-blockchain_data.extra_bytes_size_data_length],blockchain_data.extra_bytes_size_data_length);
  tmp_value = strtol(blockchain_data.extra_bytes_size_data, NULL, 16);
  if (tmp_value < 0) return XCASH_ERROR;
  blockchain_data.extra_bytes_size = varint_decode((size_t)tmp_value);
  ...
  count+= blockchain_data.reserve_bytes_size_data_length;
  memcpy(blockchain_data.reserve_bytes_size_data,&DATA[count-blockchain_data.reserve_bytes_size_data_length],blockchain_data.reserve_bytes_size_data_length);
  tmp_value = strtol(blockchain_data.reserve_bytes_size_data, NULL, 16);
  if (tmp_value < 0) return XCASH_ERROR;
  blockchain_data.reserve_bytes_size = varint_decode((size_t)tmp_value);

  // Skip full BLOCKCHAIN_RESERVED_BYTES parsing in this patch (assumed safe)
  if ((message_copy1 = strstr(DATA,BLOCKCHAIN_RESERVED_BYTES_END)) == NULL && (message_copy1 = strstr(DATA,BLOCKCHAIN_RESERVED_BYTES_DATA_HASH)) == NULL)
  {
    ERROR_PRINT("Invalid reserved bytes segment");
    return XCASH_ERROR;
  }
  count = strnlen(DATA,BUFFER_SIZE) - strnlen(message_copy1,BUFFER_SIZE) + (strstr(DATA,BLOCKCHAIN_RESERVED_BYTES_END) != NULL ? sizeof(BLOCKCHAIN_RESERVED_BYTES_END)-1 : sizeof(BLOCKCHAIN_RESERVED_BYTES_DATA_HASH)-1);
  ...
  // transaction_amount
  blockchain_data.transaction_amount_data_length = (strnlen(DATA, BUFFER_SIZE) - count) >= 2 ? 2 : 0;
  count+= blockchain_data.transaction_amount_data_length;
  memcpy(blockchain_data.transaction_amount_data,&DATA[count-blockchain_data.transaction_amount_data_length],blockchain_data.transaction_amount_data_length);
  tmp_value = strtol(blockchain_data.transaction_amount_data, NULL, 16);
  if (tmp_value < 0 || tmp_value > MAXIMUM_TRANSACATIONS_PER_BLOCK) return XCASH_ERROR;
  blockchain_data.transaction_amount = varint_decode((size_t)tmp_value);
...
int network_block_string_to_blockchain_data(const char* DATA, const char* BLOCK_HEIGHT, const int BLOCK_VERIFIERS_TOTAL)
{
  ...
  DEBUG_PRINT("Initializing variables and clearing memory");
  memset(block_height,0,sizeof(block_height));
  memset(data2,0,sizeof(data2));
  memset(&blockchain_data, 0, sizeof(blockchain_data));
  memset(blockchain_data.network_version_data,0,sizeof(blockchain_data.network_version_data));
  memset(blockchain_data.timestamp_data,0,sizeof(blockchain_data.timestamp_data));
  memset(blockchain_data.previous_block_hash_data,0,sizeof(blockchain_data.previous_block_hash_data));
  memset(blockchain_data.nonce_data,0,sizeof(blockchain_data.nonce_data));
  memset(blockchain_data.block_reward_transaction_version_data,0,sizeof(blockchain_data.block_reward_transaction_version_data));
  memset(blockchain_data.unlock_block_data,0,sizeof(blockchain_data.unlock_block_data));
  memset(blockchain_data.block_reward_input_data,0,sizeof(blockchain_data.block_reward_input_data));
  memset(blockchain_data.vin_type_data,0,sizeof(blockchain_data.vin_type_data));
  memset(blockchain_data.block_height_data,0,sizeof(blockchain_data.block_height_data));
  memset(blockchain_data.block_reward_output_data,0,sizeof(blockchain_data.block_reward_output_data));
  memset(blockchain_data.block_reward_data,0,sizeof(blockchain_data.block_reward_data));
  memset(blockchain_data.stealth_address_output_tag_data,0,sizeof(blockchain_data.stealth_address_output_tag_data));
  memset(blockchain_data.stealth_address_output_data,0,sizeof(blockchain_data.stealth_address_output_data));
  memset(blockchain_data.extra_bytes_size_data,0,sizeof(blockchain_data.extra_bytes_size_data));
  memset(blockchain_data.transaction_public_key_tag_data,0,sizeof(blockchain_data.transaction_public_key_tag_data));
  memset(blockchain_data.transaction_public_key_data,0,sizeof(blockchain_data.transaction_public_key_data));
  memset(blockchain_data.extra_nonce_tag_data,0,sizeof(blockchain_data.extra_nonce_tag_data));
  memset(blockchain_data.reserve_bytes_size_data,0,sizeof(blockchain_data.reserve_bytes_size_data));
  memset(blockchain_data.blockchain_reserve_bytes.block_producer_delegates_name_data,0,sizeof(blockchain_data.blockchain_reserve_bytes.block_producer_delegates_name_data));
  memset(blockchain_data.blockchain_reserve_bytes.block_producer_delegates_name,0,sizeof(blockchain_data.blockchain_reserve_bytes.block_producer_delegates_name));
  memset(blockchain_data.blockchain_reserve_bytes.block_producer_public_address_data,0,sizeof(blockchain_data.blockchain_reserve_bytes.block_producer_public_address_data));
  memset(blockchain_data.blockchain_reserve_bytes.block_producer_public_address,0,sizeof(blockchain_data.blockchain_reserve_bytes.block_producer_public_address));
  memset(blockchain_data.blockchain_reserve_bytes.block_producer_node_backup_count_data,0,sizeof(blockchain_data.blockchain_reserve_bytes.block_producer_node_backup_count_data));
  ...
  memset(blockchain_data.blockchain_reserve_bytes.block_producer_backup_nodes_names_data,0,sizeof(blockchain_data.blockchain_reserve_bytes.block_producer_backup_nodes_names_data));
  ...
  memset(blockchain_data.blockchain_reserve_bytes.vrf_secret_key_data,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_secret_key_data));
  memset(blockchain_data.blockchain_reserve_bytes.vrf_secret_key,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_secret_key));
  memset(blockchain_data.blockchain_reserve_bytes.vrf_public_key_data,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_public_key_data));
  memset(blockchain_data.blockchain_reserve_bytes.vrf_public_key,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_public_key));
  memset(blockchain_data.blockchain_reserve_bytes.vrf_alpha_string_data,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_alpha_string_data));
  memset(blockchain_data.blockchain_reserve_bytes.vrf_alpha_string,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_alpha_string));
  memset(blockchain_data.blockchain_reserve_bytes.vrf_proof_data,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_proof_data));
  memset(blockchain_data.blockchain_reserve_bytes.vrf_proof,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_proof));
  memset(blockchain_data.blockchain_reserve_bytes.vrf_beta_string_data,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_beta_string_data));
  memset(blockchain_data.blockchain_reserve_bytes.vrf_beta_string,0,sizeof(blockchain_data.blockchain_reserve_bytes.vrf_beta_string));
  ...
  for (count = 0; (int)count < BLOCK_VERIFIERS_TOTAL_AMOUNT; count++)
  {
    ...
    memset(blockchain_data.blockchain_reserve_bytes.block_verifiers_vrf_public_key[count],0,sizeof(blockchain_data.blockchain_reserve_bytes.block_verifiers_vrf_public_key[count]));
    memset(blockchain_data.blockchain_reserve_bytes.block_verifiers_random_data[count],0,sizeof(blockchain_data.blockchain_reserve_bytes.block_verifiers_random_data[count]));
    memset(blockchain_data.blockchain_reserve_bytes.block_verifiers_random_data_text[count],0,sizeof(blockchain_data.blockchain_reserve_bytes.block_verifiers_random_data_text[count]));
    memset(blockchain_data.blockchain_reserve_bytes.next_block_verifiers_public_address_data[count],0,sizeof(blockchain_data.blockchain_reserve_bytes.next_block_verifiers_public_address_data[count]));
    memset(blockchain_data.blockchain_reserve_bytes.next_block_verifiers_public_address[count],0,sizeof(blockchain_data.blockchain_reserve_bytes.next_block_verifiers_public_address[count]));
    memset(blockchain_data.blockchain_reserve_bytes.block_validation_node_signature_data[count],0,sizeof(blockchain_data.blockchain_reserve_bytes.block_validation_node_signature_data[count]));
    memset(blockchain_data.blockchain_reserve_bytes.block_validation_node_signature[count],0,sizeof(blockchain_data.blockchain_reserve_bytes.block_validation_node_signature[count]));
  }
  memset(blockchain_data.ringct_version_data,0,sizeof(blockchain_data.ringct_version_data));
  memset(blockchain_data.transaction_amount_data,0,sizeof(blockchain_data.transaction_amount_data));
  for (count = 0; count < MAXIMUM_TRANSACATIONS_PER_BLOCK; count++)
  {
    memset(blockchain_data.transactions[count],0,sizeof(blockchain_data.transactions[count]));
  }
  count = 0;

  DEBUG_PRINT("Parsing network_version");
  blockchain_data.network_version_data_length = sizeof(NETWORK_VERSION)-1;
  count+= blockchain_data.network_version_data_length;
  memcpy(blockchain_data.network_version_data,DATA,blockchain_data.network_version_data_length);

  DEBUG_PRINT("Parsing timestamp");
  blockchain_data.timestamp_data_length = TIMESTAMP_LENGTH;
  count+= blockchain_data.timestamp_data_length;
  memcpy(blockchain_data.timestamp_data,&DATA[count-blockchain_data.timestamp_data_length],blockchain_data.timestamp_data_length);
  tmp_value = strtol(blockchain_data.timestamp_data, NULL, 16);
  ...
  DEBUG_PRINT("Parsing previous_block_hash");
  blockchain_data.previous_block_hash_data_length = BLOCK_HASH_LENGTH;
  count+= blockchain_data.previous_block_hash_data_length;
  memcpy(blockchain_data.previous_block_hash_data,&DATA[count-blockchain_data.previous_block_hash_data_length],blockchain_data.previous_block_hash_data_length);

  DEBUG_PRINT("Parsing nonce");
  blockchain_data.nonce_data_length = sizeof(BLOCK_PRODUCER_NETWORK_BLOCK_NONCE)-1;
  count+= blockchain_data.nonce_data_length;
  memcpy(blockchain_data.nonce_data,&DATA[count-blockchain_data.nonce_data_length],blockchain_data.nonce_data_length);

  DEBUG_PRINT("Parsing block_reward_transaction_version");
  blockchain_data.block_reward_transaction_version_data_length = sizeof(BLOCK_REWARD_TRANSACTION_VERSION)-1;
  count+= blockchain_data.block_reward_transaction_version_data_length;
  memcpy(blockchain_data.block_reward_transaction_version_data,&DATA[count-blockchain_data.block_reward_transaction_version_data_length],blockchain_data.block_reward_transaction_version_data_length);

  DEBUG_PRINT("Parsing unlock_block");
  if (strncmp(BLOCK_HEIGHT,"0",1) == 0)
  {
    if (get_current_block_height(block_height) == 0)
    {
  ...
  DEBUG_PRINT("Parsing block_reward_input");
  blockchain_data.block_reward_input_data_length = sizeof(BLOCK_REWARD_INPUT)-1;
  count+= blockchain_data.block_reward_input_data_length;
  memcpy(blockchain_data.block_reward_input_data,&DATA[count-blockchain_data.block_reward_input_data_length],blockchain_data.block_reward_input_data_length);

  DEBUG_PRINT("Parsing vin_type");
  blockchain_data.vin_type_data_length = sizeof(VIN_TYPE)-1;
  count+= blockchain_data.vin_type_data_length;
  memcpy(blockchain_data.vin_type_data,&DATA[count-blockchain_data.vin_type_data_length],blockchain_data.vin_type_data_length);

  DEBUG_PRINT("Parsing block_height");
  blockchain_data.block_height_data_length = number > VARINT_DECODED_VALUE_END_2_BYTE ? 8 : 6;
  count+= blockchain_data.block_height_data_length;
  memcpy(blockchain_data.block_height_data,&DATA[count-blockchain_data.block_height_data_length],blockchain_data.block_height_data_length);
  tmp_value = strtol(blockchain_data.block_height_data, NULL, 16);
  ...
  DEBUG_PRINT("Parsing block_reward_output");
  blockchain_data.block_reward_output_data_length = sizeof(BLOCK_REWARD_OUTPUT)-1;
  count+= blockchain_data.block_reward_output_data_length;
  memcpy(blockchain_data.block_reward_output_data,&DATA[count-blockchain_data.block_reward_output_data_length],blockchain_data.block_reward_output_data_length);

  DEBUG_PRINT("Parsing block_reward");
  if ((data3 = strstr(DATA,BLOCKCHAIN_RESERVED_BYTES_START)) == NULL && (data3 = strstr(DATA,BLOCKCHAIN_RESERVED_BYTES_DATA_HASH)) == NULL)
  {
    ERROR_PRINT("Invalid network_block_string, Invalid block_reward");
    return XCASH_ERROR;
  ...
  DEBUG_PRINT("Parsing stealth_address_output_tag");
  blockchain_data.stealth_address_output_tag_data_length = sizeof(STEALTH_ADDRESS_OUTPUT_TAG)-1;
  count+= blockchain_data.stealth_address_output_tag_data_length;
  memcpy(blockchain_data.stealth_address_output_tag_data,&DATA[count-blockchain_data.stealth_address_output_tag_data_length],blockchain_data.stealth_address_output_tag_data_length);

  DEBUG_PRINT("Parsing stealth_address_output");
  blockchain_data.stealth_address_output_data_length = STEALTH_ADDRESS_OUTPUT_LENGTH;
  count+= blockchain_data.stealth_address_output_data_length;
  memcpy(blockchain_data.stealth_address_output_data,&DATA[count-blockchain_data.stealth_address_output_data_length],blockchain_data.stealth_address_output_data_length);

  DEBUG_PRINT("Parsing extra_bytes_size");
  blockchain_data.extra_bytes_size_data_length = 2;
  count+= blockchain_data.extra_bytes_size_data_length;
  memcpy(blockchain_data.extra_bytes_size_data,&DATA[count-blockchain_data.extra_bytes_size_data_length],blockchain_data.extra_bytes_size_data_length);
  tmp_value = strtol(blockchain_data.extra_bytes_size_data, NULL, 16);
  ...
  DEBUG_PRINT("Parsing transaction_public_key_tag");
  blockchain_data.transaction_public_key_tag_data_length = sizeof(TRANSACTION_PUBLIC_KEY_TAG)-1;
  count+= blockchain_data.transaction_public_key_tag_data_length;
  memcpy(blockchain_data.transaction_public_key_tag_data,&DATA[count-blockchain_data.transaction_public_key_tag_data_length],blockchain_data.transaction_public_key_tag_data_length);

  DEBUG_PRINT("Parsing transaction_public_key");
  blockchain_data.transaction_public_key_data_length = TRANSACTION_PUBLIC_KEY_LENGTH;
  count+= blockchain_data.transaction_public_key_data_length;
  memcpy(blockchain_data.transaction_public_key_data,&DATA[count-blockchain_data.transaction_public_key_data_length],blockchain_data.transaction_public_key_data_length);

  DEBUG_PRINT("Parsing extra_nonce_tag");
  blockchain_data.extra_nonce_tag_data_length = sizeof(EXTRA_NONCE_TAG)-1;
  count+= blockchain_data.extra_nonce_tag_data_length;
  memcpy(blockchain_data.extra_nonce_tag_data,&DATA[count-blockchain_data.extra_nonce_tag_data_length],blockchain_data.extra_nonce_tag_data_length);

  DEBUG_PRINT("Parsing reserve_bytes_size");
  blockchain_data.reserve_bytes_size_data_length = 2;
  count+= blockchain_data.reserve_bytes_size_data_length;
  memcpy(blockchain_data.reserve_bytes_size_data,&DATA[count-blockchain_data.reserve_bytes_size_data_length],blockchain_data.reserve_bytes_size_data_length);
  tmp_value = strtol(blockchain_data.reserve_bytes_size_data, NULL, 16);
  ...
  DEBUG_PRINT("Parsing reserved bytes section marker");
  if ((message_copy1 = strstr(DATA,BLOCKCHAIN_RESERVED_BYTES_END)) == NULL && (message_copy1 = strstr(DATA,BLOCKCHAIN_RESERVED_BYTES_DATA_HASH)) == NULL)
  {
    ERROR_PRINT("Invalid reserved bytes segment");
    return XCASH_ERROR;
  ...
  DEBUG_PRINT("Parsing ringct_version");
  blockchain_data.ringct_version_data_length = sizeof(RINGCT_VERSION)-1;
  count+= blockchain_data.ringct_version_data_length;
  memcpy(blockchain_data.ringct_version_data,&DATA[count-blockchain_data.ringct_version_data_length],blockchain_data.ringct_version_data_length);

  DEBUG_PRINT("Parsing transaction_amount");
  blockchain_data.transaction_amount_data_length = (strnlen(DATA, BUFFER_SIZE) - count) >= 2 ? 2 : 0;
  count+= blockchain_data.transaction_amount_data_length;
  memcpy(blockchain_data.transaction_amount_data,&DATA[count-blockchain_data.transaction_amount_data_length],blockchain_data.transaction_amount_data_length);
  tmp_value = strtol(blockchain_data.transaction_amount_data, NULL, 16);
  ...
  DEBUG_PRINT("Parsing transactions");
  for (number = 0; number < blockchain_data.transaction_amount; number++)
  {
    count+= TRANSACTION_LENGTH;
    memcpy(blockchain_data.transactions[number],&DATA[count-TRANSACTION_LENGTH],TRANSACTION_LENGTH);
  ...
  DEBUG_PRINT("network_block_string_to_blockchain_data complete");
  return XCASH_OK;
}
