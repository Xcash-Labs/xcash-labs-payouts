...
---------------------------------------------------------------------------------------------------------*/
bool generate_and_request_vrf_data_sync__OLD__(char** message) {
...
bool generate_and_request_vrf_data_sync(char** message) {
  ...
  INFO_PRINT("VRF: starting generate_and_request_vrf_data_sync()");

  // Decode pk from config string
  if (!hex_to_byte_array(vrf_public_key, pk_bin, sizeof(pk_bin))) {
    ERROR_PRINT("Invalid hex format for public key");
    return false;
  }
  DEBUG_HEX("vrf_public_key(pk_bin)", pk_bin, sizeof(pk_bin));
  ...
  // OPTIONAL (diagnostic only): derive pk from sk and warn if mismatch (does not change behavior)
  {
    unsigned char pk_from_sk[crypto_vrf_PUBLICKEYBYTES] = {0};
    crypto_vrf_sk_to_pk(pk_from_sk, secret_key_data);
    if (memcmp(pk_from_sk, pk_bin, crypto_vrf_PUBLICKEYBYTES) != 0) {
      // Non-fatal: continue using pk_bin as before; this just explains "works elsewhere but not here"
      ERROR_PRINT("VRF WARN: pk_from_sk != configured vrf_public_key on this node");
      DEBUG_HEX("pk_from_sk(derived)", pk_from_sk, sizeof(pk_from_sk));
      DEBUG_HEX("pk_bin(configured)", pk_bin, sizeof(pk_bin));
    }
  }

  // Decode previous block hash (first 32 bytes of alpha)
  if (!hex_to_byte_array(previous_block_hash, previous_block_hash_bin, 32)) {
    ERROR_PRINT("Failed to decode previous block hash");
    return false;
  }
  ...
  // Height (8 bytes LE) at offset 32
  uint64_t block_height = strtoull(current_block_height, NULL, 10);
  uint64_t height_le = htole64(block_height);
  memcpy(alpha_input_bin + 32, &height_le, sizeof(height_le));

  // Producer/public key (32 bytes) at offset 40 (as before)
  memcpy(alpha_input_bin + 40, pk_bin, 32);

  // Log α segments clearly
  INFO_PRINT("VRF: block_height=%" PRIu64, block_height);
  DEBUG_HEX("alpha[0..31]   prev_hash", alpha_input_bin, 32);
  DEBUG_HEX("alpha[32..39]  height_le", alpha_input_bin + 32, 8);
  DEBUG_HEX("alpha[40..71]  pk_bin   ", alpha_input_bin + 40, 32);
  ...
  DEBUG_HEX("vrf_proof(80)", vrf_proof, crypto_vrf_PROOFBYTES);
  ...
  // Hex encodings for message
  for (i = 0, offset = 0; i < crypto_vrf_PROOFBYTES; i++, offset += 2)
    snprintf(vrf_proof_hex + offset, 3, "%02x", vrf_proof[i]);
  for (i = 0, offset = 0; i < crypto_vrf_OUTPUTBYTES; i++, offset += 2)
    snprintf(vrf_beta_hex + offset, 3, "%02x", vrf_beta[i]);

  // Self-verify (diagnostic logs on failure)
  {
    unsigned char computed_beta[crypto_vrf_OUTPUTBYTES] = {0};
    int rc = crypto_vrf_verify(computed_beta, pk_bin, vrf_proof, alpha_input_bin, sizeof(alpha_input_bin));
    if (rc != 0) {
      DEBUG_PRINT("Failed to verify the VRF proof for this node (self-check)");
      // Dump everything needed to reproduce α and verify
      DEBUG_HEX("verify.pk_bin", pk_bin, crypto_vrf_PUBLICKEYBYTES);
      DEBUG_HEX("verify.alpha ", alpha_input_bin, sizeof(alpha_input_bin));
      DEBUG_HEX("verify.proof ", vrf_proof, crypto_vrf_PROOFBYTES);
      return false;
    }

    if (memcmp(computed_beta, vrf_beta, crypto_vrf_OUTPUTBYTES) != 0) {
      DEBUG_PRINT("Failed to match the computed VRF beta for this node");
      DEBUG_HEX("computed_beta", computed_beta, crypto_vrf_OUTPUTBYTES);
      DEBUG_HEX("vrf_beta     ", vrf_beta, crypto_vrf_OUTPUTBYTES);
      return false;
    }
  }

  // Compose outbound message (unchanged contents)
  *message = create_message_param(
      XMSG_BLOCK_VERIFIERS_TO_BLOCK_VERIFIERS_VRF_DATA,
      "public_address", xcash_wallet_public_address,
      "vrf_public_key", vrf_public_key,
  ...
  INFO_PRINT("VRF: generate_and_request_vrf_data_sync() complete");
  return true;
}
